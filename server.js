...
async function ensureFonts() {
  ...
  const fonts = [
    { url: 'https://github.com/ArtifexSoftware/urern/raw/master/resources/fonts/ttf/DejaVuSans.ttf', path: FONT_PATH },
    { url: 'https://github.com/ArtifexSoftware/urern/raw/master/resources/fonts/ttf/DejaVuSans-Bold.ttf', path: FONT_BOLD_PATH }
  ];
...
function downloadFile(url, dest) {
  return new Promise((resolve, reject) => {
    // Use simpler approach - embed font data
    resolve();
  });
}
...
app.post('/api/export-pdf', async (req, res) => {
  ...
  try {
    ...
    // Register fonts - use built-in Helvetica which has limited but safer character support
    // For full Unicode support, we'll sanitize text
    ...
    const langTitles = {
      ...
      ru: { report: 'KRYONIS Analiticheskiy Otchet', question: 'VOPROS', synthesis: 'SINTEZ DIRIZHYORA', agents: 'OTVETY AGENTOV', generated: 'Sgenerirovano KRYONIS Lazy Susan PRO' },
      et: { report: 'KRYONIS Analyysiraport', question: 'KUSIMUS', synthesis: 'DIRIGENDI SUNTEES', agents: 'AGENTIDE VASTUSED', generated: 'Genereeritud KRYONIS Lazy Susan PRO' }
    };
    ...
    const agentCodes = {
      'Architect': 'KRYONIS-A',
      'Red Team': 'KRYONIS-B',
      'Synthesizer': 'KRYONIS-G',
      'Facts': 'KRYONIS-D',
      'Style': 'KRYONIS-E',
      'Futurist': 'KRYONIS-Z',
      'Devil\'s Advocate': 'KRYONIS-H'
    };
    ...
    const transliterate = (text) => {
      const map = {
        ...
        'К':'K','Л':'L','М':'M','Н':'N','О':'O','П':'P','Р':'R','С':'S','Т':'T','У':'U','Ф':'F',
        'Х':'Kh','Ц':'Ts','Ч':'Ch','Ш':'Sh','Щ':'Sch','Ъ':'','Ы':'Y','Ь':'','Э':'E','Ю':'Yu','Я':'Ya',
        'ä':'ae','ö':'oe','ü':'ue','õ':'o','Ä':'Ae','Ö':'Oe','Ü':'Ue','Õ':'O',
        'š':'sh','ž':'zh','Š':'Sh','Ž':'Zh'
      };
    ...
    const processText = (text) => {
      return sanitizeText(transliterate(text));
    };

    // Header
    doc.fontSize(24).fillColor('#1B4D3E').text(titles.report, { align: 'center' });
    doc.moveDown(0.5);
    ...
    doc.fontSize(11).fillColor('#333').text(processText(question).substring(0, 500));
    doc.moveDown(1);

    // Synthesis
    doc.fontSize(14).fillColor('#1B4D3E').text(titles.synthesis, { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor('#333').text(processText(synthesis), {
      align: 'justify',
      lineGap: 2
    });
    doc.moveDown(1);
    ...
    for (const agent of agents) {
      ...
      doc.fontSize(9).fillColor('#8B6914').text(processText(agent.role || ''));
      doc.fontSize(9).fillColor('#333').text(processText(agent.response).substring(0, 1200), {
        align: 'justify',
        lineGap: 1
      });
      doc.moveDown(0.8);
