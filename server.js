...
// Transliteration function for PDF
function transliterate(text) {
  const map = {
    ...
    'К':'K','Л':'L','М':'M','Н':'N','О':'O','П':'P','Р':'R','С':'S','Т':'T','У':'U','Ф':'F',
    'Х':'Kh','Ц':'Ts','Ч':'Ch','Ш':'Sh','Щ':'Sch','Ъ':'','Ы':'Y','Ь':'','Э':'E','Ю':'Yu','Я':'Ya',
    'ä':'a','ö':'o','ü':'u','õ':'o','Ä':'A','Ö':'O','Ü':'U','Õ':'O',
    'ž':'zh','š':'sh','Ž':'Zh','Š':'Sh'
  };
...
app.post('/api/export-pdf', async (req, res) => {
  ...
  try {
    ...
    // Header
    doc.fontSize(24).fillColor('#1B4D3E').text('KRYONIS Analysis Report', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor('#666').text(new Date(timestamp).toLocaleString(), { align: 'center' });
    doc.moveDown(1);

    // Question
    doc.fontSize(12).fillColor('#8B6914').text('QUESTION:', { continued: false });
    doc.fontSize(11).fillColor('#333').text(cleanForPDF(question).substring(0, 500));
    doc.moveDown(1);

    // Synthesis
    doc.fontSize(14).fillColor('#1B4D3E').text('CONDUCTOR\'S SYNTHESIS', { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor('#333').text(cleanForPDF(synthesis), {
      align: 'justify',
      lineGap: 2
    });
    doc.moveDown(1);
    ...
    // Agents
    doc.addPage();
    doc.fontSize(14).fillColor('#1B4D3E').text('AGENT RESPONSES', { underline: true });
    doc.moveDown(0.5);
    ...
    for (const agent of agents) {
      ...
      doc.fontSize(9).fillColor('#8B6914').text(cleanForPDF(agent.role || ''));
      doc.fontSize(9).fillColor('#333').text(cleanForPDF(agent.response).substring(0, 1000), {
        align: 'justify',
        lineGap: 1
      });
      doc.moveDown(0.8);
    ...
    // Footer
    doc.fontSize(8).fillColor('#999').text('Generated by KRYONIS Lazy Susan PRO', 50, doc.page.height - 50);
