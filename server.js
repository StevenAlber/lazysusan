...
app.post('/api/export-pdf', async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(11).fillColor('#333').text(question.substring(0, 500));
    doc.moveDown(1);

    // Synthesis
    doc.fontSize(14).fillColor('#1B4D3E').text('CONDUCTOR\'S SYNTHESIS', { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor('#333').text(synthesis.replace(/\*\*/g, '').replace(/##/g, '').replace(/###/g, ''), {
      align: 'justify',
      lineGap: 2
    });
    doc.moveDown(1);
    ...
    for (const agent of agents) {
      ...
      doc.fontSize(9).fillColor('#8B6914').text(agent.role || '');
      doc.fontSize(9).fillColor('#333').text(agent.response.replace(/\*\*/g, '').substring(0, 800), {
        align: 'justify',
        lineGap: 1
      });
      doc.moveDown(0.8);
